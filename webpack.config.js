...
module.exports = function makeWebpackConfig() {
     ...
     */
    config.entry = isTest ? {} : {
        main: './src/app/chatter-async.module.ts'
    };

    config.resolve = {
        extensions: ['', '.ts', '.js'],
        modulesDirectories: ['node_modules', "vendor_components", "src/app"],
    ...
    config.output = isTest ? {} : {
        ...
        publicPath: isProd ? '/' : 'http://localhost:3000/app/',

        // Filename for entry points
        // Only adds hash in build mode
        filename: isProd ? '[name].[hash].js' : '[name].module.js',

        // Filename for non-entry points
        // Only adds hash in build mode
        chunkFilename: isProd ? '[name].[hash].js' : '[name].module.js'
    };
    ...
    config.module = {
        ...
        loaders: [{ test: /\.ts$/, exclude: /node_modules|bower_components|cli.d.ts/, loaders: ['ng-annotate', 'awesome-typescript-loader'] },
        {
            ...
            loader: isTest ? 'null' : "style-loader!css-loader?-url"
    ...
    // if (!isTest) {
    //Girish Always skip rendering index.html for now 
    if (1 == 2) {
